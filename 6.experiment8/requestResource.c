...
//instances of resources
	...
	need[n][r];
	for(i=0;i<r;i++){
	        printf("\nEnter the instance of %d resource:",i);
		scanf("%d",&instances[i]);
	}
...
/*
1. request[i] <= need[i]     goto 2
2. request[i] <= available   goto 3
3. available = available - request[i]
   allocation[i]=allocation[i] + request[i]
   need[i]=need[i]-request[i]	
   
   check if new state is safer or not
*/
     int request[n+1][r];
     ...
     for(i=0;i<n;i++) {
     	...
     	int process=0;	
        printf("\n enter the process which want to check immediate permission granted");
        scanf("%d",&process);
        
        for(i=0;i<r;i++) {
        if(request[process][i] <= need[process][i])
        {
         if(request[process][i] <= available) {
            available -= request[process][i];
            allocated[process][i] +=request[process][i];
